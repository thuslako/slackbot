...
const initSlack = (app) => {
    ...
    slack.command("/oncall-report", async ({ ack, respond, command }) => {
        ...
        try {
            ...
            let sentryText = "";
            console.log(`[mcp] calling sentry_list_issues_org with org=${env_1.env.SENTRY_ORG}, environment=${environment}, statsPeriod=${statsPeriod}`);
            const t0 = Date.now();
            const se = await ensureMcpClient("sentry");
            const seRes = await se.callTool({
                name: "sentry_list_issues_org",
                arguments: { org: env_1.env.SENTRY_ORG, environment, statsPeriod, since: sinceIso, until: untilIso, limitPerProject: 20 }
            ...
            if (sentryText.length > 16000)
                sentryText = sentryText.slice(0, 16000);
